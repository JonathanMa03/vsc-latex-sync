...
\maketitle
\noindent \emph{This course was taught by Dr. Yanxun Xu in the Spring 2026 Semester. The notes are transcribed by Jonathan Ma and may contain errors and omitted content, so any issues are to be directed towards me.}
...
\subsection{Reading: Gibbs Sampling for Stick-Breaking Priors}

\subsubsection*{Stick-Breaking Random Measures}
...
\subsubsection*{Finite vs Infinite Construction}
...
\subsubsection*{Dirichlet Process Special Case}
...
\subsubsection*{Generalized Gibbs Sampling}
...
\subsubsection*{Pitman–Yor Extension}
...
\subsubsection*{Properties of PPMx}
...
\subsubsection*{Choice of Similarity Models}
...
\subsubsection*{Posterior Inference under PPMx}
...
\section*{Extension: Feature Modeling across multiple groups}
...
\begin{itemize}
    \item What topics exist
    \item Which topics appear in each article
    \item how topic usage differs across news sources
\end{itemize}
We do this by using the Beta Process and IBP framework 
\[
    B \sim BP(c, H), \quad B=\sum_{k=1}^\infty w_k \delta_{\theta_k}
\]
Where $\theta_k$ is the latent topic and $w_k$ is the probability that the topic appears. Each article independently chooses which topics it uses based on
\[
    z_{ik} \sim \Bern(w_k)
\]
where $z_{ik}=1$ if article $i$ uses topic $k$ and 0 otherwise.

\subsection*{Limitation of the Vanilla Beta Process}
Under the standard model 
\[
    P(z_{ik}=1)=w_k \quad \forall i
\]
All articles share the same topic probabilities, which implies that CNN articles use the same topic frequencies as BBC articles, and this is unrealistic as different news outlets ephasize different topics. We want a model with a \emph{shared global pool of topics} but a group-specific topic probability:
\begin{itemize}
    \item \emph{Global feature measure} defines a shared collection of latent topics
    \[
        B_0 \sim BP(c_0,H) \quad 
        B_0 = \sum_{k=1}^\infty p_k \delta_{\theta_k}
    \]
    \item \emph{Group specific feature measures}: Each news source $j$ gets its own topic probabilities
    \[
        B_j | B_0 \sim BP(c_1, B_0) \quad
        B_j = \sum_{k=1}^\infty w_{jk} \delta_{\theta_k}, w_{jk}\sim \BetaDist(c_1 p_k, c_1 (1-p_k))
    \]
    \item \emph{Data Generation}: set of topics appearing in article $i$ from news source $j$
    \[
        X_{ji} | B_j \sim BeP(B_j) \quad z_{jik} | B_j \sim \text{Bernoulli}(B_j)
    \]
\end{itemize}

\section{From IBP to Count-Valued Feature Models}

\begin{itemize}
\item Recall Beta--Bernoulli construction:
\[
B \sim \text{BP}(c, H), \quad X_i \mid B \sim \text{BeP}(B)
\]

\item Produces binary feature usage:
\[
z_{ik} \in \{0,1\}
\]
...
\item Limitation:
\begin{itemize}
\item Many real datasets are count-valued (not binary)
\end{itemize}

\item Example:
\begin{itemize}
\item Gene expression (RNA-seq)
\item Counts of mRNA transcripts per cell
\end{itemize}

\item Motivation:
\begin{itemize}
\item Need models that extend IBP to count data
\end{itemize}
\end{itemize}

\section{Negative Binomial Factor Analysis (NBFA)}

\begin{itemize}
\item Data:
\[
X_{vj} = \text{count of gene } v \text{ in cell } j
\]
...
\item Components:
\begin{itemize}
\item $\phi_{vk}$: gene loading for factor $k$
\item $\theta_{kj}$: activity of factor $k$ in sample $j$
\item $p_j$: dispersion parameter
\end{itemize}

\item Interpretation:
\begin{itemize}
\item Counts arise from multiple latent factors
\end{itemize}
...
\section{Double Feature Allocation (DFA)}

\begin{itemize}
\item Goal:
\begin{itemize}
\item Model relationships between two sets (e.g., patients and symptoms)
\end{itemize}

\item Idea:
\begin{itemize}
\item Allow overlapping clusters in both dimensions
\end{itemize}
...
\section{Latent Disease Interpretation}

\begin{itemize}
\item Latent disease:
\begin{itemize}
\item Unobserved condition generating symptoms
\end{itemize}

\item Structure:
\begin{itemize}
\item Patient--disease relationships
\item Disease--symptom relationships
\end{itemize}

\item Observed data:
\begin{itemize}
\item Patient symptom matrix
\end{itemize}
\end{itemize}

\section{Patient--Disease (PD) Model}

\begin{itemize}
\item Feature allocation via IBP:
\[
...
\item Interpretation:
\begin{itemize}
\item Column $k$ = latent disease
\item $\alpha_{ik}=1$ means patient $i$ has disease $k$
\end{itemize}

\item Properties:
\begin{itemize}
\item Finite number of active diseases for finite $n$
\end{itemize}
\end{itemize}

\section{Symptom--Disease (SD) Model}

\begin{itemize}
\item Conditional on PD model:
\[
...
\gamma_{lk} =
\begin{cases}
-1 & \text{disease } k \text{ decreases symptom } l \\
0 & \text{no relation} \\
+1 & \text{disease } k \text{ increases symptom } l
\end{cases}
\]
\end{itemize}
...
\item Model:
\[
y_{il} \sim \text{Categorical}\left(
M e^{\alpha_i^\top W^- \gamma_l^- + \eta_l^-},
\; M,
\; M e^{\alpha_i^\top W^+ \gamma_l^+ + \eta_l^+}
\right)
\]
...
\item Components:
\begin{itemize}
\item $W^-, W^+$: weight matrices
\item $\eta^- , \eta^+$: noise terms
\item $M$: normalization constant
\end{itemize}
\end{itemize}
...
\item MCMC inference:
\begin{itemize}
\item Sequential updates of $A$
\item Slow convergence
\end{itemize}

\item Large $n$:
\begin{itemize}
\item Many rows (patients)
\item Many features (columns)
\item High memory cost
\end{itemize}
\end{itemize}
...
\item Idea:
\begin{enumerate}
\item Split data into subsets
\item Run MCMC independently
\item Combine posterior samples
\end{enumerate}

\item Limitation for IBP:
\begin{itemize}
\item Feature labels are not aligned across subsets
\item Cannot average discrete feature matrices
\end{itemize}
\end{itemize}

\section{Modified Consensus Monte Carlo}

\begin{itemize}
\item Fix:
\begin{enumerate}
\item Introduce overlapping subsets (anchors)
\item Align features across subsets
\item Combine via anchor comparison
\end{enumerate}

\item Result:
\begin{itemize}
\item More scalable IBP inference
\end{itemize}
\end{itemize}

\chapter{Polya Tree Priors}

\section{Prior Construction}

\begin{itemize}
\item So far we have introduced:
\begin{itemize}
\item Dirichlet process (DP): prior on random probability measures for clustering
\item Indian buffet process (IBP): prior on binary latent feature matrices
\end{itemize}

\item Alternative approach: construct priors via recursive partitioning (Polya tree)

\item Intuition:
\begin{itemize}
\item A histogram approximates a distribution via bins
\item Fixed partitions are too rigid
\item Polya tree recursively refines partitions
\end{itemize}
\end{itemize}

\section{Ingredients of a Polya Tree}

A Polya tree requires:

\begin{itemize}
\item Recursive partition of the sample space
\item Random branching probabilities at each split
\item Path rule defining bin probabilities
\end{itemize}

\section{Recursive Partition Tree}

\begin{itemize}
\item Let $(\Omega, \mathcal{B})$ be a measurable space

\item Define a sequence of partitions:
\[
...
\begin{itemize}
\item $\pi_0 = \{\Omega\}$
\item $\pi_{m+1}$ refines $\pi_m$
\item Each set splits into two disjoint children
\end{itemize}
\end{itemize}

\subsection{Binary Indexing}
...
\item Each node indexed by binary string $\epsilon \in E^m$
...
\section{Polya Tree Definition}

\begin{itemize}
\item Define branching probabilities:
\[
...
\item Then:
\[
G(B_{\epsilon 0}) = G(B_\epsilon) Y_\epsilon, \quad
G(B_{\epsilon 1}) = G(B_\epsilon)(1 - Y_\epsilon)
\]
...
\item Probability of node:
\[
G(B_\epsilon) = \prod_{j=1}^m
Y_{\epsilon_1\cdots\epsilon_{j-1}}^{1-\epsilon_j}
(1 - Y_{\epsilon_1\cdots\epsilon_{j-1}})^{\epsilon_j}
\]
\end{itemize}

\section{Tailfree Processes}
...
\item A prior is \textbf{tailfree} if:
\begin{itemize}
\item collections at each level are independent:
\[
\{Y_\emptyset\}, \{Y_0,Y_1\}, \{Y_{00},Y_{01},Y_{10},Y_{11}\}, \dots
\]
\end{itemize}

\item Polya tree is tailfree
\end{itemize}

\section{Dirichlet Process as a Special Case}
...
\item With:
\[
\alpha_\epsilon = \alpha_{\epsilon 0} + \alpha_{\epsilon 1}, \quad
\alpha_\epsilon = c\,H(B_\epsilon)
\]
...
\item If:
\[
\alpha_0 = \alpha_{00} + \alpha_{01}, \quad
\alpha_1 = \alpha_{10} + \alpha_{11}
\]

\item Then:
\[
(G(B_{00}), G(B_{01}), G(B_{10}), G(B_{11}))
\sim
\text{Dirichlet}(\alpha_{00}, \alpha_{01}, \alpha_{10}, \alpha_{11})
\]

\item By induction:
\[
(G(B_\epsilon) : |\epsilon| = m)
\sim
\text{Dirichlet}(\alpha_\epsilon : |\epsilon|=m)
\]
\end{itemize}
...
\begin{itemize}
\item Polya tree = distribution over distributions
\item Built via recursive Beta splits
\item Provides more local control than DP
\item Can encode prior beliefs about regions of $\Omega$
\end{itemize}

\section{Absolute Continuity of Polya Trees}

\begin{itemize}
\item Absolute continuity result (Kraft, 1964):

If the concentration parameters grow quadratically,
\[
\alpha_{\epsilon_1 \cdots \epsilon_m} = c m^2,
\]
...
At level $m$,
\[
Y_\epsilon \sim \text{Beta}(\alpha_m, \alpha_m), \quad \alpha_m = c m^2
\]

\begin{itemize}
\item Mean: $1/2$
\item Variance: $\mathcal{O}(1/\alpha_m)$
\end{itemize}

\item As $m \to \infty$:
\begin{itemize}
\item Splits become increasingly balanced
\item Mass divides proportionally to region size
\item Density emerges in the limit
\end{itemize}
...
\[
G = \sum_{k=1}^\infty \pi_k \delta_{\theta_k}
\quad \Rightarrow \quad \text{discrete}
\]

Polya tree (with growing $\alpha$): continuous.
\end{itemize}

\section{Effect of Parameters}

\begin{itemize}
\item $c$ controls variability around base measure $G_0$:

\begin{itemize}
\item $c \uparrow$ $\Rightarrow$ tighter around $G_0$
\item $c \downarrow$ $\Rightarrow$ rougher, more variable draws
\end{itemize}
\end{itemize}

\section{Prior Centering}
...
\begin{itemize}
\item $\Pi$: recursive partition
\item $\mathcal{A} = \{(\alpha_{\epsilon 0}, \alpha_{\epsilon 1})\}$: Beta parameters
\end{itemize}
...
\section{Centering Method 1: Quantile Partition}

\begin{itemize}
\item Use symmetric splits:
\[
\alpha_{\epsilon 0} = \alpha_{\epsilon 1}
\]

\item Define $\Pi$ using quantiles of $G_0$:

\begin{itemize}
\item Split at median: $G_0^{-1}(1/2)$
\item Then quartiles: $G_0^{-1}(1/4), G_0^{-1}(3/4)$
\item Continue recursively
\end{itemize}

\item Ensures structure aligns with $G_0$
\end{itemize}

\section{Centering Method 2: Match Split Probabilities}

\begin{itemize}
\item Fix partition $\Pi$

\item Choose parameters such that:
\[
\frac{\alpha_{\epsilon 0}}{\alpha_{\epsilon 0} + \alpha_{\epsilon 1}}
=
G_0(B_{\epsilon 0} \mid B_\epsilon)
\]

\item Equivalent:
\[
\alpha_{\epsilon 0} \propto G_0(B_{\epsilon 0} \mid B_\epsilon), \quad
\alpha_{\epsilon 1} \propto G_0(B_{\epsilon 1} \mid B_\epsilon)
\]
\end{itemize}

\section{Why This Centers the Prior}
...
\item Taking expectation:
\[
\mathbb{E}[G(B_{\epsilon 0}) \mid G(B_\epsilon)]
= G(B_\epsilon) \mathbb{E}[Y_\epsilon]
\]

\item Since:
\[
\mathbb{E}[Y_\epsilon] = \frac{\alpha_{\epsilon 0}}{\alpha_{\epsilon 0} + \alpha_{\epsilon 1}}
= G_0(B_{\epsilon 0} \mid B_\epsilon)
\]
...
\[
\mathbb{E}[G(B_{\epsilon 0}) \mid G(B_\epsilon)]
= G(B_\epsilon) G_0(B_{\epsilon 0} \mid B_\epsilon)
\]

\item Recursively:
\[
\mathbb{E}[G(B_{\epsilon_1 \cdots \epsilon_m})]
= G_0(B_{\epsilon_1 \cdots \epsilon_m})
\]
\end{itemize}
...
\item Count data:
\[
n_{\epsilon 0} = \#\{x_i \in B_{\epsilon 0}\}, \quad
n_{\epsilon 1} = \#\{x_i \in B_{\epsilon 1}\}
\]

\item Posterior:
\[
Y_\epsilon \mid x_{1:n}
\sim
\text{Beta}(\alpha_{\epsilon 0} + n_{\epsilon 0}, \alpha_{\epsilon 1} + n_{\epsilon 1})
\]
\end{itemize}

\section{Posterior Predictive Sampling}

\begin{itemize}
\item To sample $x_{n+1}$:

\item Step 1: traverse tree

\begin{itemize}
\item At node $\epsilon$, sample:
\[
Y_\epsilon \mid x_{1:n} \sim \text{Beta}(\alpha_{\epsilon 0}^*, \alpha_{\epsilon 1}^*)
\]
\item Choose left/right branch
\end{itemize}

\item Step 2: stop at a leaf and sample:
\[
...
\begin{itemize}
\item Each $x_i$ follows a path down the tree
\item Contribution depends on shared path with previous points
\end{itemize}

\item Leads to tractable likelihood via Beta-Binomial recursion
\end{itemize}

\section{Polya Tree: Marginal Distribution}

\begin{itemize}
\item The marginal density after integrating out $G$:
\[
p(x_1,\dots,x_n)
=
\prod_{i=1}^n G_0(x_i)
\prod_{j=2}^n
\prod_{m=1}^{m^*(x_j)}
\frac{\alpha_{\epsilon_m(x_j)}^*}{\alpha_{\epsilon_m(x_j)}}
\cdot
\frac{\alpha_{\epsilon_m 0(x_j)} + \alpha_{\epsilon_m 1(x_j)}}
{\alpha_{\epsilon_m 0(x_j)}^* + \alpha_{\epsilon_m 1(x_j)}^*}
\]

\item Notation:
\begin{itemize}
\item $\epsilon_m(x_j)$: index of level-$m$ cell containing $x_j$
\item $m^*(x_j)$: first level where $x_j$ is isolated
\item $\alpha^*$: posterior-updated parameters
\end{itemize}

\item Interpretation:
\begin{itemize}
\item Base contribution: $\prod_i G_0(x_i)$
\item Correction: depends on shared paths in the tree
\end{itemize}
\end{itemize}

\section{Mixture of Polya Trees (MPT)}

\begin{itemize}
\item Issue: single PT produces rough densities (non-differentiable)

\item Fix: introduce mixture over centering measures:
\[
...
\item Effect:
\begin{itemize}
\item Smooths discontinuities
\item Produces continuous predictive densities
\end{itemize}

\item Key idea: averaging over $\theta$ removes boundary artifacts
\end{itemize}

\section{Motivation: Joint Modeling}
...
\item Challenge:
\begin{itemize}
\item heterogeneous populations
\item multiple treatments
\item parametric assumptions too restrictive
\end{itemize}
...
\begin{itemize}
\item $[T]$: nonparametric (Polya tree)
\item $[Y \mid T]$: regression model
\end{itemize}
\end{itemize}

\section{Multivariate Polya Tree}
...
\begin{itemize}
\item Step 1: dyadic partition in $\mathbb{R}$
\[
B_0(m,k) = (\Phi^{-1}((k-1)/2^m), \Phi^{-1}(k/2^m)]
\]

\item Step 2: form rectangles in $\mathbb{R}^p$
\[
B_0(m,\mathbf{k}) = \prod_{j=1}^p B_0(m,k_j)
\]

\item Step 3: affine transform
\[
...
\item Result:
\begin{itemize}
\item recursive partition aligned with Gaussian geometry
\item partitions $\mathbb{R}^p$ into $2^{mp}$ cells at level $m$
\end{itemize}
\end{itemize}

\section{Rubbery Polya Tree}

\begin{itemize}
\item Problem: standard PT has independent splits $\Rightarrow$ discontinuities

\item Idea: introduce dependence across splits

\item Construction (finite example):
\[
Y_{00} \sim \text{Beta}(\alpha_2,\alpha_2), \quad
Z_{00} \mid Y_{00} \sim \text{Binomial}(\delta_{00}, Y_{00})
\]
\[
...
\item Effect:
\begin{itemize}
\item neighboring splits positively correlated
\item smoother densities
\end{itemize}
\end{itemize}

\section{Linear Dependent Tailfree Process (LDTP)}

\begin{itemize}
\item Goal: regression version of PT

\item Replace fixed splits with covariate-dependent splits:
\[
...
Y_{x,e0}
=
\frac{\exp(x^\top \beta_{e0})}{1 + \exp(x^\top \beta_{e0})}
\]

\item Result:
\begin{itemize}
\item distribution varies smoothly with $x$
\item predictor affects entire shape of density
\end{itemize}
\end{itemize}

\section{Why LDTP Helps}
...
\item Limitation:
\begin{itemize}
\item only mean/variance change
\item shape fixed (Gaussian)
\end{itemize}
...
\item Advantage:
\begin{itemize}
\item full distribution adapts with $x$
\item captures skewness, multimodality, tail behavior
\end{itemize}
\end{itemize}

\section{LDTP Case Study: Growth Curve Analysis}
...
\item Issue:
\begin{itemize}
\item distribution shape evolves with $x$
\item skewness, tails, modality change
\end{itemize}
...
\item Advantage:
\begin{itemize}
\item models full conditional distribution
\item allows shape to vary with $x$
\end{itemize}
\end{itemize}

\section{Dependent Multivariate Polya Trees}
...
\item Requirement:
\begin{itemize}
\item each $G_x$ is marginally a PT
\item dependence across $x$
\end{itemize}

\item Replace scalar split:
\[
...
\item $Q$: controls dependence across covariates
\end{itemize}

\section{Multivariate Beta Process (MBP)}
...
\item Properties:
\begin{itemize}
\item $Y_x \sim \text{Beta}(\alpha_0,\alpha_1)$
\item dependence across nearby $x$
\end{itemize}

\item Key idea:
\begin{itemize}
\item share Gamma components
\end{itemize}
...
\item Dependence:
\begin{itemize}
\item overlapping Gamma terms $\Rightarrow$ correlation
\end{itemize}
\end{itemize}

\section{MBP Construction}

\begin{itemize}
\item Gamma process on augmented space
...
\item Regions:
\[
S_x^0 = \{(t,y): 0 < y < \alpha_0 q_x(t)\}, \quad
S_x^1 = \{(t,y): -\alpha_1 q_x(t) < y < 0\}
\]
...
\begin{itemize}
\item marginal Beta
\item dependence via overlap of $S_x^0, S_x^1$
\end{itemize}
\end{itemize}

\section{Finite-Dimensional Representation for MCMC}
...
\item Sampling:
\begin{itemize}
\item search first atom in region $S_{x_i}$
\item set $Z_i = \mathbb{I}(\zeta_{ij} > 0)$
\end{itemize}

\item Key result:
\begin{itemize}
\item MBP reduces to DP-based Polya urn
\item tractable MCMC
\end{itemize}
\end{itemize}

\section{Mixture of Polya Trees Revisited}
...
\item Effect:
\begin{itemize}
\item smooth densities
\item removes partition artifacts
\end{itemize}
\end{itemize}

\section{Polya Trees in Joint Modeling}
...
\begin{itemize}
\item $T$: PT prior
\item $Y \mid T$: regression
\end{itemize}

\item Advantage:
\begin{itemize}
\item avoids restrictive parametric survival models
\item captures heterogeneous populations
\end{itemize}
\end{itemize}
...
\item Standard PT:
\begin{itemize}
\item independent splits
\item rough densities
\end{itemize}

\item MPT:
\begin{itemize}
\item mixture over centering measures
\item smoother densities
\end{itemize}

\item LDTP:
\begin{itemize}
\item predictor-dependent splits
\item full conditional density regression
\end{itemize}

\item Dependent PT:
\begin{itemize}
\item preserves Beta marginals
\item uses MBP for dependence
\end{itemize}

\item MBP:
\begin{itemize}
\item induces dependence via shared Gamma processes
\item enables DP-based computation
\end{itemize}
\end{itemize}

\section{Alternative Construction via MCMC}

\begin{itemize}
\item Goal: introduce branching indicators at a node
\[
...
\item Interpretation:
\begin{itemize}
\item $Z_i = 1$ $\Rightarrow$ left child
\item $Z_i = 0$ $\Rightarrow$ right child
\end{itemize}

\item Given split probability at $x_i$:
\[
Z_i \mid Y_{x_i} \sim \text{Bernoulli}(Y_{x_i})
\]

\item Instead of sampling $Y_{x_i}$ directly:
\begin{itemize}
\item sample induced branching variables
\[
Z = (Z_1,\dots,Z_m)
\]
\item sufficient for tree recursion
\end{itemize}
\end{itemize}

\section{DP Representation}
...
\item Generate latent points:
\[
(\xi_{ij}, \zeta_{ij}) \in S, \quad
(\xi_{ij}, \zeta_{ij}) \mid D \overset{iid}{\sim} D_{x_1,\dots,x_m}
\]
\end{itemize}

\section{Polya Urn Sampling}

\begin{itemize}
\item Let $w_r = (\xi,\zeta)$

\item Polya urn update:
\[
w_r \mid w_1,\dots,w_{r-1}
\sim
\frac{\nu(S)}{\nu(S)+r-1}\nu
+
...
\item Interpretation:
\begin{itemize}
\item hit in $S_{x_i}^0$ $\Rightarrow Z_i=1$
\item hit in $S_{x_i}^1$ $\Rightarrow Z_i=0$
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item MBP / LDTP sampling reduces to:
\begin{itemize}
\item DP-based Polya urn
\item sampling latent points instead of split probabilities
\end{itemize}

\item Key advantage:
\begin{itemize}
\item avoids direct sampling of $\{Y_{x_i}\}$
\item enables scalable MCMC
\end{itemize}
\end{itemize}

\chapter{Bayesian Nonparametric Survival Analysis}

\section{Survival Analysis: Setup}

\begin{itemize}
\item Time-to-event data: model the random event time $T$.

\item Observations (with censoring):
\[
Y_i = \min(T_i, C_i), \quad
\delta_i = \mathbb{I}(T_i < C_i)
\]

\item Likelihood (independent censoring):
\[
...
\[
S(t) = \Pr(T > t), \quad
\lambda(t) = \frac{f(t)}{S(t)}, \quad
\Lambda(t) = \int_0^t \lambda(u)\,du
\]
\end{itemize}

\section{Semiparametric Survival Models}

\subsection{Proportional Hazards (PH)}
...
\begin{itemize}
\item $\lambda_0(t)$: baseline hazard (nonparametric)
\item $\exp(x^\top \beta)$: multiplicative covariate effect
\item Hazard ratio:
\[
...
\begin{itemize}
\item Covariates rescale time:
\[
S_x(t) = S_0(\exp(-x^\top \beta)t)
\]
\item BNP extension: place a DPM prior on $\epsilon$
\end{itemize}

\subsection{Proportional Odds (PO)}

\[
\frac{S_x(t)}{1 - S_x(t)} =
\frac{S_0(t)}{1 - S_0(t)} \exp(-x^\top \beta)
\]

\begin{itemize}
\item Allows time-varying hazard ratios
\end{itemize}

\section{Bayesian Semiparametric PH Models}

\begin{itemize}
\item Place BNP prior on baseline hazard:
\[
...
\item Common priors:
\begin{itemize}
\item Gamma process (continuous hazard)
\item Beta process (discrete jumps)
\item Piecewise exponential models
\end{itemize}

\item Advantage:
\begin{itemize}
\item full posterior over survival curve
\end{itemize}
\end{itemize}

\section{Neutral-to-the-Right (NTR) Processes}

\begin{itemize}
\item A random CDF $F$ is NTR if
\[
1 - F(t_1), \;
\frac{1 - F(t_2)}{1 - F(t_1)}, \dots
\]
are independent.
...
\item Key property:
\begin{itemize}
\item conjugacy under censoring
\end{itemize}

\item Special case:
\begin{itemize}
\item Dirichlet process $\subset$ NTR
\end{itemize}
\end{itemize}

\section{Fully Nonparametric Survival Regression}

\begin{itemize}
\item Goal: model full conditional family
\[
\{ S(t \mid x) : x \in \mathcal{X} \}
\]

\item No restriction to:
\begin{itemize}
\item proportional hazards
\item log-linear time scaling
\end{itemize}
\end{itemize}

\subsection{ANCOVA-DDP}
...
\item $\{G_x\}$ follows a dependent Dirichlet process

\item Interpretation:
\begin{itemize}
\item full distribution shape varies with $x$
\item borrowing strength across nearby covariates
\end{itemize}
\end{itemize}

\subsection{Linear Dependent Tailfree Process (LDTP)}

\begin{itemize}
\item Replace PT splits with covariate-dependent ones:
\[
G_x(B_{e0} \mid B_e)
=
\frac{\exp(x^\top \beta_{e0})}{1 + \exp(x^\top \beta_{e0})}
...
\item Key property:
\begin{itemize}
\item full conditional density changes shape with $x$
\end{itemize}
\end{itemize}

\section{Why BNP Survival Models}
...
\item Semiparametric models:
\begin{itemize}
\item restrict covariate effects to $\beta$
\end{itemize}
...
\item multimodality
\item skewness
\item time-varying effects
\end{itemize}
\end{itemize}
\end{itemize}

\section{Fully Nonparametric Survival Regression: LDTP (Interpretation)}

\subsection{Interpretation I}

\begin{itemize}
\item \textbf{Why fully nonparametric:}
\begin{itemize}
\item Separate parameter $\beta_{e0}$ at each node $e$ of the infinite tree
\item Covariate effect is distributed across infinitely many parameters
\item The family $\{G_x\}$ can represent arbitrarily complex conditional distributions
\end{itemize}

\item \textbf{How covariates enter:}
\[
G_x(B_{e0} \mid B_e) = \frac{\exp(x^\top \beta_{e0})}{1 + \exp(x^\top \beta_{e0})}
\]

\begin{itemize}
\item Covariates act locally at each resolution level
\item Can reshape distribution differently across the support
\item Allows asymmetric effects (e.g., left vs right tail)
\end{itemize}
\end{itemize}

\subsection{Interpretation II}

\begin{itemize}
\item \textbf{Prior on coefficients:}
\[
\beta_{e0} \sim \mathcal{N}\left(0, g(X^\top X)^{-1}\right),
\quad g = \frac{2n}{c}
\]

\begin{itemize}
\item Shrinks toward zero $\Rightarrow$ equal splits (regularization)
\item Prevents overfitting at deep tree levels
\end{itemize}

\item \textbf{Comparison with ANCOVA-DDP:}
\begin{itemize}
\item Both define $\{G_x\}$ fully nonparametrically
\item ANCOVA-DDP:
\begin{itemize}
...
\item LDTP:
\begin{itemize}
\item tree-based partition
\item exact (non-mixture) representation
\end{itemize}
\item Different computation and theoretical properties
\end{itemize}
\end{itemize}

\section{Alternative BNP Construction: DDP-GP}

\subsection{Model Construction}

\begin{itemize}
\item Replace linear mean with a Gaussian process:
\[
...
\[
C(Z_i, Z_\ell) =
\sigma_0^2 \exp\left(-\sum_{d=1}^D \frac{(Z_{id} - Z_{\ell d})^2}{\lambda_d^2}\right)
+ \delta_{i\ell} J^2
\]
\end{itemize}
...
\begin{itemize}
\item Similar covariates $\Rightarrow$ similar function values
\item $\lambda_d$: length-scale controls smoothness
\item $J^2$: jitter for numerical stability
\end{itemize}

\section{DDP-GP Model for Survival Data}
...
\item $Y_i = \log(T_i)$
\item $Z_i$: covariates
\item $\delta_i$: censoring indicator
\end{itemize}
...
\item Likelihood (right censoring):
\[
\mathcal{L}(\theta \mid D_n)
=
\prod_{i=1}^n
\left\{ f_{Z_i}(Y_i \mid \theta)^{\delta_i}
\left[1 - F_{Z_i}(Y_i \mid \theta)\right]^{1-\delta_i} \right\}
\]

\item Interpretation:
\begin{itemize}
\item Observed events $\Rightarrow$ density contribution
\item Censored observations $\Rightarrow$ survival contribution
\end{itemize}
\end{itemize}

\section{Comparison of BNP Survival Models}

\begin{itemize}
\item \textbf{ANCOVA-DDP}
\begin{itemize}
\item Mixture-based
\item Smooth densities
\item Clustering interpretation
\end{itemize}

\item \textbf{LDTP}
\begin{itemize}
\item Tree-based
\item Explicit control of distribution shape
\item Highly flexible local structure
\end{itemize}

\item \textbf{DDP-GP}
\begin{itemize}
\item Function-based dependence
\item Smooth variation over covariates
\item Strong geometric interpretation via kernels
\end{itemize}
\end{itemize}
